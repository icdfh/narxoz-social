...
import { makeWsUrl } from "../../utils/ws";         // <── NEW
...
export default function ChatRoom({ chatId }) {
  ...
  const token = useSelector(s => s.auth.token);   // `access`-токен строкой
  ...
  const loadChats = useCallback(async () => {
    ...
    if (missing.length) {
      ...
      await Promise.all(missing.map(async id => {
        try {
          map[id] = (await apiClient.get(`/users/profile/${id}/`)).data;
        } catch {
          map[id] = { is_online: false };
        }
      }));
      setUMap(map);
    }
  ...
  const connectWS = () => {
    try {
      wsRef.current = new WebSocket(makeWsUrl(`chat/${chatId}/`, token));
    } catch {
    ...
    sock.onclose = e => {
      if (!e.wasClean) retryRef.current = setTimeout(connectWS, 2000);
    };
    sock.onerror = () => {}; // без логов в консоль
  };

  /* ───── init + re‑init при смене chatId/token ───── */
  useEffect(() => {
    let mounted = true;
    (async () => {
      await loadChats();
  ...
  const sendWS = payload => {
    ...
    sock.readyState === WebSocket.OPEN ? run() : sock.addEventListener("open", run, { once: true });
  };
  ...
  /* ───── share message (из MessageBubble) ───── */
  const handleShareMenu = msg => {
    if (msg.id.includes("-")) {
      setToast("⏳ Подождите, сообщение сохраняется");
          ...
          <div className="hdr-right">
            ...
            {menuOpen && peerProfile && (
              <div className="chat-header-menu" ref={menuRef}>
                <button onClick={() => { navigate(`/profile/${peerProfile.id}`); setMenuOpen(false); }}>Просмотреть профиль</button>
