/* ─────────────────────────────────────
   MessageBubble — одно сообщение чата
   ───────────────────────────────────── */
  ...
  return img ? (
    ...
    <div className="file-card">
      ...
      <div className="file-meta">
        ...
        <a className="file-dl" href={url} download={safe}>Скачать</a>
      </div>
    </div>
  );
};
...
export default function MessageBubble({
  msg, currentUserId, sender,
  formatTime, onEdit, onDelete, onShare
}) {
  ...
  useEffect(()=>{
    ...
    if(typeof msg.share_id === "object"){ acceptPost(msg.share_id); return; }

    apiClient.get(`/posts/${msg.share_id}/`)
      .then(r=>acceptPost(r.data))
      .catch(()=>setPost(null));
  },[msg]);

  useEffect(()=>{
    if(!post || avatar) return;
    apiClient.get(`/users/profile/${post.author_id}/`)
  ...
  const renderContent = () => {
      ...
      return(
        <div className={`msg-shared ${isOwn?"own":"other"}`}>
          <div className="shared-header">
            <span className="shared-author">@{msg.sender_nickname}</span> поделился:
          </div>
    ...
    if(msg.share_type==="profile" && msg.share_id){
      ...
      return(
        <div className={`msg-shared ${isOwn?"own":"other"}`}>
          <div className="shared-header">
            <span className="shared-author">@{msg.sender_nickname}</span> поделился профилем:
          </div>
          <div className="shared-profile">
            <img src={fullUrl(p.avatar_path||p.avatar_url)||"/avatar.jpg"}
    ...
    if(msg.share_type==="post"){
      ...
      return(
        <div className={`msg-shared ${isOwn?"own":"other"}`}>
          <div className="shared-header">
            <span className="shared-author">@{msg.sender_nickname}</span> поделился постом:
          </div>
      ...
      <div className={`msg-bubble ${isOwn?"own":"other"}`}>
        ...
        {menuOpen&&(
          <div ref={menuRef} className="context-menu" style={{top:menuPos.y,left:menuPos.x}}>
            {onShare && <div onClick={()=>{setMenuOpen(false);onShare(msg);}}>🔄 Поделиться</div>}
            {onEdit  && <div onClick={()=>{setMenuOpen(false);onEdit(msg);}}>✏️ Редактировать</div>}
            {onDelete&& <div onClick={()=>{setMenuOpen(false);setAskDel(true);}}>🗑️ Удалить</div>}
          </div>
        )}
        ...
        {askDel&&(
          <div className="confirm-popover">
            ...
            <button onClick={()=>{setAskDel(false);onDelete?.(msg);}}>Да</button>
          </div>
        )}
      </div>
    </div>
