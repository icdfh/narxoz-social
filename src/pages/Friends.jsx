...
export default function Friends() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const decInc = (incRes.data || []).filter((r) => r.status === "declined");
      const decOut = (outRes.data || []).filter((r) => r.status === "declined");
      setDeclined([...decInc, ...decOut]);
    } catch (e) {
  ...
  const resend = (r) => {
    ...
    dispatch(fetchNotifications());
    apiClient
      .delete(`/friends/cancel/${r.id}/`)
      .then(() => {
        const tid =
          r.from_user.id === currentUser.id ? r.to_user.id : r.from_user.id;
  ...
  const renderList = () => {
    if (loading) return <div className="friends-empty">Загрузка…</div>;
    ...
    switch (activeTab) {
      ...
      case "Outgoing":
        ...
        return outgoing.map((r) =>
          renderCard(r.to_user, "ожидает подтверждения", (
            <button
  ...
  return (
    <div className="friends-page">
      ...
      <div className="friends-list">{renderList()}</div>
          ...
          <button
            onClick={() => {
              setReportParams({
                ...
                avatar: buildUrl(
                  menuUser.avatar_path || menuUser.avatar_url
                ),
